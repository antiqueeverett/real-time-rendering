...
bool SimpleShaders::loadVertGeomFragShaders(const char* vertexShaderFilename, const char* geometryShaderFilename, const char* fragmentShaderFilename){
	// Create empty shader object (vertex shader)
	mVertexShader = glCreateShader(GL_VERTEX_SHADER);
	...
	shaderSource = readFile( geometryShaderFilename );
	sourcePtr = shaderSource.c_str();

	// Attach shader code
	glShaderSource(mGeometryShader, 1, &sourcePtr, NULL);	

	// Compile
	glCompileShader(mGeometryShader);
	printShaderInfoLog(mGeometryShader);
...
GLuint SimpleShaders::getUniform(std::string uniform) const{
	auto search = mUniforms.find(uniform);
