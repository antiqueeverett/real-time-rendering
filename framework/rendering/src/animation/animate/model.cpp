...
animation_Mesh Model::processMesh(aiMesh* mesh, const aiScene* scene)
{
	...
	return animation_Mesh(vertices, indices, textures, bones_id_weights_for_each_vertex);
}
...
vector<Texture> Model::LoadMaterialTexture(aiMaterial* mat, aiTextureType type, string type_name)
{
	...
	for (uint i = 0; i < mat->GetTextureCount(type); i++)
	{
		...
		Texture texture;
		texture.id = Loader::loadImageToTexture(filename.c_str()); // return prepaired openGL texture
